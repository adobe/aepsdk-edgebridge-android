...
class ListenerGenericTrackRequestContent extends ExtensionListener {
	 ...
	 */
	ListenerGenericTrackRequestContent(final ExtensionApi extensionApi, final String type, final String source) {
		super(extensionApi, type, source);
	}

	/**
	 * Method that gets called when event with event type {@link EdgeBridgeConstants.EventType#GENERIC_TRACK}
	 * and with event source {@link EdgeBridgeConstants.EventSource#REQUEST_CONTENT} is dispatched through eventHub.
	 *
	...
	public void hear(final Event event) {
		if (event == null || event.getEventData() == null) {
			MobileCore.log(
				LoggingMode.DEBUG,
				LOG_TAG,
				"ListenerGenericTrackRequestContent - Event or Event data is null. Ignoring the event."
			);
			return;
		}
		...
		if (parentExtension == null) {
			MobileCore.log(
				LoggingMode.DEBUG,
				LOG_TAG,
				"ListenerGenericTrackRequestContent - The parent extension, associated with this listener is null. Ignoring the event."
			);
			return;
		}
		...
		parentExtension
			...
			.execute(
				new Runnable() {
					@Override
					public void run() {
						parentExtension.handleTrackRequest(event);
					}
				}
			);
	}
